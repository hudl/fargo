...
func discoverDNS(domain string) (servers []string, ttl time.Duration, err error) {
	...
	z, _ := availabilityZone()
	domain = "txt." + z + "." + dns.Fqdn(domain)
	if _, ok := dns.IsDomainName(domain); !ok {
		err = fmt.Errorf("invalid domain name: '%s' is not a domain name", domain)
		return
	}
	availabilityZoneRecords, err := findTXT(domain)
	if err != nil {
		return
	}

	for _, az := range availabilityZoneRecords {
		s, er := findTXT("txt." + dns.Fqdn(az))
		if er != nil {
			continue
		}
...
func findTXT(fqdn string) ([]string, error) {
	query := new(dns.Msg)
...
func availabilityZone() (string, error) {
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		...
		return "us-east-1", fmt.Errorf("bad response code: code %d does not indicate successful request, body=%s",
			response.StatusCode,
			body,
		)
	}
